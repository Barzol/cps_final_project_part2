...
% --- Figure 1: Trajectories in the plane ---
figure('Name','[SF] Trajectories','Position',[50 50 700 600]);
hold on; grid on; axis equal;
title(sprintf('[State Feedback]  Trajectories — %s reference', ref_type),'FontSize',13);
...
figure('Name','[SF] Position Tracking','Position',[50 50 900 500]);
...
% --- Figure 3: Formation errors ---
figure('Name','[SF] Formation Errors','Position',[50 50 900 500]);
...
sgtitle('[State Feedback]  Formation errors  \delta_i = (y_i - y_1) - (h_i - h_1)','FontSize',12);

% --- Figure 4: Physical inputs ---
figure('Name','[SF] Physical Inputs','Position',[50 50 900 600]);
for i = 1:3
    subplot(3,2,2*i-1);
...
sgtitle('[State Feedback]  Physical inputs u_i','FontSize',12);

% --- Figure 5: Formation error norms ---
figure('Name','[SF] Error Norms','Position',[50 50 700 350]);
hold on; grid on;
...
xlabel('t [s]');  ylabel('[m]');
title('[State Feedback]  Formation error norms');
legend('Location','best');
